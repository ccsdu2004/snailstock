...
void CrashHandler::GetExceptionPointers(DWORD dwExceptionCode,EXCEPTION_POINTERS** ppExceptionPointers)
{
...
void CrashHandler::CreateMiniDump(EXCEPTION_POINTERS* pExcPtrs)
{   
	...
	std::wstring dumpPath = L"crash\\";
	SHCreateDirectoryEx(NULL, dumpPath.c_str(), NULL);

	TCHAR fileName[64];
	SYSTEMTIME st;
	::GetLocalTime(&st);
	swprintf_s(fileName, L"snail.quant%04d-%02d-%02d-%02d-%02d-%02d-%02d-%02d.dmp", st.wYear, st.wMonth, st.wDay,
...
void __cdecl CrashHandler::InvalidParameterHandler(
...
int __cdecl CrashHandler::NewHandler(size_t)
{
