...
struct VarMeta_maxVar : public VarMeta
{
	...
	bool execute(int pos,ResultType& output)
	{
		...
		for (int i = pos;i > pos - n;i--)
		{
			...
			if (!ok)
				return false;
			d = std::max<double>(d,res.double_);
		}
	...
	int n;
	static ObjectRegister<VarMeta,VarMeta_maxVar,QString> dummy;
};

ObjectRegister<VarMeta,VarMeta_maxVar,QString> VarMeta_maxVar::dummy("maxVar");
