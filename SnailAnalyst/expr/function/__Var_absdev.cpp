...
struct VarMeta_absdev : public VarMeta
{
	...
	bool execute(int pos,ResultType& output)
	{
		...
		output.double_ = gsl_absdev(d.data(),d.size());
		return true;
	}

	int n;
	static ObjectRegister<VarMeta,VarMeta_absdev,QString> dummy;
};

ObjectRegister<VarMeta,VarMeta_absdev,QString> VarMeta_absdev::dummy("absdev");
