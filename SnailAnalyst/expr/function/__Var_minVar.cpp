...
struct VarMeta_minVar : public VarMeta
{
	...
	bool execute(int pos,ResultType& output)
	{
		...
		for (int i = pos;i > pos - n;i--)
		{
			...
			if (!ok)
				return false;
			d = std::min<double>(d,res.double_);
		}
	...
	int n;
	static ObjectRegister<VarMeta,VarMeta_minVar,QString> dummy;
};

ObjectRegister<VarMeta,VarMeta_minVar,QString> VarMeta_minVar::dummy("minVar");
